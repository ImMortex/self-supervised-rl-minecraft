...
class SSLHead(nn.Module):
    def __init__(self, args, upsample="vae", dim=768, output_kernel_size=(1, 1, 1)):  # changes: output_kernel_size
        ...
        self.swinViT = SwinViT(
            ...
            depths=[2, 2, 6, 2],    # changes: set to Swin-T values
            num_heads=[3, 6, 12, 24],
            mlp_ratio=4.0,
            qkv_bias=True,
            drop_rate=0.0,
        ...
        if upsample == "large_kernel_deconv":
            ...
            self.conv = nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False),
                nn.Conv3d(dim // 16, args.in_channels, kernel_size=output_kernel_size, stride=1), # changes: kernel_size
            )
