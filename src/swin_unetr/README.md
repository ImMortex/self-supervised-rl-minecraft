...
```bash
python -m torch.distributed.launch --nproc_per_node=<Num-GPUs> --master_port=11223 agent_recording_random.py
...
```bash
python -m torch.distributed.launch --nproc_per_node=8 --master_port=11223 agent_recording_random.py
...
```bash
python agent_recording_random.py --use_checkpoint --batch_size=<Batch-Size> --num_steps=<Num-Steps> --lrdecay
