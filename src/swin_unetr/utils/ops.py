...
def patch_rand_drop(args, x, x_rep=None, max_drop=0.3, max_block_sz=0.25, tolr=0.05):
    ...
    mx_blk_slices = max(int(z * max_block_sz), z)  # stcngurs bugfix: added max to ensure min value of z
    ...
    while total_pix < n_drop_pix:
        ...
        rnd_z = max(min(randint(tolr[2], mx_blk_slices) + rnd_s, z), z)  # stcngurs bugfix: added max to ensure min value of z

        if rnd_h - rnd_r > 0 and rnd_w - rnd_c > 0 and rnd_z - rnd_s > 0: # stcngurs bugfix: added condi
...
def rot_rand(args, x_s):
    ...
    for i in range(img_n):
        ...
        if orientation == 0:
            ...
            x = x.rot90(1, (1, 2)) # stcngurs bugfix: dim 2,3 replaced with 1,2
        elif orientation == 2:
            x = x.rot90(2, (1, 2)) # stcngurs bugfix: dim 2,3 replaced with 1,2
        elif orientation == 3:
            x = x.rot90(3, (1, 2)) # stcngurs bugfix: dim 2,3 replaced with 1,2
        x_aug[i] = x
        x_rot[i] = orientation
    return x_aug, x_rot
