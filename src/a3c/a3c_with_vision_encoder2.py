...
class A3CWithVisionEncoder2(nn.Module):
    """
    ...
    def __init__(self,  train_config: dict, device, model_name):
        super(A3CWithVisionEncoder2, self).__init__()
        self.device = device
        ...
        self.fc_pi1 = nn.Linear(self.vision_encoder_out_dim,  int(self.vision_encoder_out_dim/2))  # hidden layer
        self.fc_pi2 = nn.Linear(int(self.vision_encoder_out_dim/2), action_dim)  # actor output layer (before softmax)

        # classification head v
        self.fc_v1 = nn.Linear(self.vision_encoder_out_dim, int(self.vision_encoder_out_dim/2))  # hidden layer
        self.fc_v2 = nn.Linear(int(self.vision_encoder_out_dim/2), 1)  # critic output layer
    ...
    def forward(self, x_2d_image, x_tensor_state_seq):
        ...
        pi_logits = self.fc_pi1(x)
        pi_logits = self.fc_pi2(pi_logits)  # actor output without softmax activation

        value = self.fc_v1(x)
        value = self.fc_v2(value)  # critic output, a.k.a. reward value for the state (approx. value function output)
